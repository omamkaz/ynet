...
class CardTitle(ft.Container):
    def __init__(self, page: ft.Page, atype: int | str = 0, **kwargs):
        ...
        self.content = ft.Row(
            controls=[
                ...
                ft.Column(
                    controls=[
                        ft.Text(size=14.5, color=ft.Colors.WHITE, text_align="right"),
                        ft.Text(text_align="right", color=ft.Colors.WHITE70, size=14),
                    ],
                    expand=True,
    ...
    def toggle_loading_mode(self, on: bool) -> None:
        ...
        self.loading_mode.current.src_base64 = (
            LottieFiles.loading_carga if on else LottieFiles.down_arrow
        )
